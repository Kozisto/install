...
install_ubuntu_debs () {
	...
	apt-get install -qq -y git automake libtool unzip build-essential qt5-default qt5-qmake qtbase5-dev-tools qttools5-dev-tools libqt5webkit5-dev libgstreamer0.10-dev libgstreamer-plugins-base0.10-dev mesa-common-dev >> /dev/null
}
...
download_deps () {
	...
	wget -q https://www.cryptopp.com/cryptopp565.zip --no-check-certificate
	# Oracle Berkeley DB 
...
unpuck_deps () {
	...
	cd $DOWNLOADS
	unzip -q cryptopp565.zip -d cryptopp565
...
build_thegcccoind () {
	echo "Compile TheGCCcoin-Qt"
	cd $GCC_ROOT
	cd TheGCCcoin-source-code
	qmake USE_UPNP=- 
	make   > /dev/null
	chmod +x TheGCCcoin-Qt
	cp TheGCCcoin-Qt ~
	cp $PREFIX/lib/libdb_cxx-4.8.so /usr/lib/
	cp $PREFIX/lib/libevent-2.1.so.6 /usr/lib/
	cd ~
}
set -e
install_ubuntu_debs
